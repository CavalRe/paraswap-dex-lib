...
async function checkOnChainPricing(
  ...
  // Normally you can get it from cavalreMultiswap.Iface or from eventPool.
  ...
  const readerResult = (
    await cavalreMultiswap.dexHelper.multiContract.methods
...
async function testPricingOnNetwork(
  ...
  const pools = await cavalreMultiswap.getPoolIdentifiers(
  ...
  const poolPrices = await cavalreMultiswap.getPricesVolume(
    networkTokens[srcTokenSymbol],
    networkTokens[destTokenSymbol],
    amounts,
    side,
  ...
  if (cavalreMultiswap.hasConstantPriceLargeAmounts) {
